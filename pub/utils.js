...
function iriToCollectionInfoFactory(
  domain,
  routes,
  pActor,
  pActivity,
  ...
  tests.push((iri) => {
    ...
    return (
      match && {
        name: "collections",
        actor: match[+!isActorFirst],
        id: match[+isActorFirst],
      }
    );
  });
...
function userAndIdToIRIFactory(domain, route, userParam, param) {
  ...
  return (user, id) => {
    ...
    return `https://${domain}${route
      .replace(param, id)
      .replace(userParam, user)}`.toLowerCase();
  };
}
...
function validateOwner(object, actor) {
  ...
  if (
    ...
    object.attributedTo[0] === actor.id
  ) {
    return true;
  }
  ...
  if (object.type === "Collection" || object.type === "OrderedCollection") {
    ...
    if (
      actorStreamNames.some((c) => actor[c] && actor[c].includes(object.id))
    ) {
      return true;
    }
    // custom collections
    if (
      actor.streams?.[0] &&
      Object.values(actor.streams[0]).includes(object.id)
    ) {
      return true;
    }
  }
...
const coreContexts = {
  "https://w3id.org/security/v1": {
    ...
    document: JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../vocab/security.json"))
    ),
  },
  "https://www.w3.org/ns/activitystreams": {
    ...
    document: JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../vocab/as.json"))
    ),
  },
};
