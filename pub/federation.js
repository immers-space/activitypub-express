...
function deliver(actorId, activity, address, signingKey) {
  ...
  const digest = crypto.createHash("sha256").update(activity).digest("base64");
  return request({
    method: "POST",
    url: address,
    headers: {
...
async function queueForDelivery(actor, activity, addresses) {
  ...
  await this.store.deliveryEnqueue(
    actor.id,
    outgoingBody,
    addresses,
    actor._meta.privateKey
  );
...
async function runDelivery() {
  ...
  if (toDeliver.waitUntil) {
    const wait = Math.min(
      toDeliver.waitUntil.getTime() - Date.now(),
      maxTimeout
    );
