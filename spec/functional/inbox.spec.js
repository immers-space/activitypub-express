  ...
  describe('post', function () {
    ...
    it('errors invalid activities', function (done) {
      request(app)
        ...
        .expect(400, 'Invalid activity', _ => done())
    })
    ...
    it('errors on unknown actor', function (done) {
      request(app)
        ...
        .set('Content-Type', 'application/activity+json')
        .send(activity)
        .expect(404, '\'noone\' not found on this instance', _ => done())
    })
    // activity save
    ...
    it('forwards from inbox', async function () {
      ...
      const requestValidated = new Promise(done => {
    ...
    describe('accept', function () {
      ...
      it('fires accept event', async function () {
        ...
        const callbackReceived = new Promise(done => {
          app.once('apex-inbox', msg => {
      ...
      it('rejects accept from non-recipients of original activity', async function () {
        ...
        await apex.store.saveActivity(follow)
        // The original test appeared to effectively ignore
        // the callback because the 403 HTTP status ended
        // the test
        //
        // const callbackReceived = new Promise(done => {
        //   app.once('apex-inbox', msg => {
      ...
      it('updates accepted activity', async function () {
        const callbackReceived = new Promise((done, reject) => {
