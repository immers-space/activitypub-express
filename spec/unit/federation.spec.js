...
describe("federation", function () {
  ...
  describe("delivery queueing", function () {
    ...
    beforeEach(async function () {
      ...
      addresses = [
        "https://ignore.com/bob/inbox",
        "https://ignore.com/sally/inbox",
      ];
    ...
    it("adds queued items to db", async function () {
      const queued = await apex.store.db
    ...
    it("dequeues items in FIFO order", async function () {
      await apex.queueForDelivery(testUser, body, [
        "https://ignore.com/lee/inbox",
      ]);
      ...
      const standard = [...addresses, "https://ignore.com/lee/inbox"].map(
        (address) => ({
          actorId: testUser.id,
          address,
          body: apex.stringifyPublicJSONLD(body),
  ...
  describe("background delivery process", function () {
    ...
    it("continues delivering until queue is empty", async function (done) {
      ...
      setTimeout(() => {
        ...
        expect(apex.deliver).toHaveBeenCalledWith(
          testUser.id,
          bodyString,
          addresses[0],
    ...
    it("retries failed delivery", async function (done) {
      ...
      setTimeout(() => {
        ...
        expect(apex.deliver.calls.argsFor(3)).toEqual([
          testUser.id,
          bodyString,
          addresses[0],
          testUser._meta.privateKey,
