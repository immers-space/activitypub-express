...
describe("utils", function () {
  ...
  describe("jsonld context caching", function () {
    ...
    it("fetches and caches new contexts", async function () {
      nock("https://mocked.com").get("/context/v1").reply(200, context);
      const doc = {
      ...
      expect(
        await apex.store.getContext("https://mocked.com/context/v1")
      ).toEqual({
    ...
    it("caches redirected contexts by original url", async function () {
      nock("https://mocked.com").get("/context/v1").reply(302, undefined, {
        Location: "http://redirect.com/context/v1",
      });
      nock("http://redirect.com").get("/context/v1").reply(200, context);
      const doc = {
        "@context": "https://mocked.com/context/v1",
        id: "https://mocked.com/s/abc123",
        customProp: "https://mocked.com/s/123abc",
  ...
  describe("validateOwner", function () {
    it("establishes collection ownerhip via actor properties", async function () {
      testUser.streams = [
        {
          custom: apex.utils.userCollectionIdToIRI(
  ...
  describe("iriToCollectionInfoFactory", function () {
    it("decode IRIs", function () {
      expect(
        apex.utils.iriToCollectionInfo("https://localhost/inbox/test")
      ).toEqual({
        name: "inbox",
        actor: "test",
      });
      expect(
        apex.utils.iriToCollectionInfo("https://localhost/followers/test")
      ).toEqual({
        name: "followers",
        actor: "test",
      });
      expect(
        apex.utils.iriToCollectionInfo("https://localhost/s/abc123/shares")
      ).toEqual({
        name: "shares",
        activity: "abc123",
      });
      expect(
        apex.utils.iriToCollectionInfo("https://localhost/u/test/c/stuff")
      ).toEqual({
        name: "collections",
        actor: "test",
        id: "stuff",
